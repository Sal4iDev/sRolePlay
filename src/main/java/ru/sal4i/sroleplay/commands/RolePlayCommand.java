...
public class RolePlayCommand extends BukkitCommand {

    private final List<UUID> cooldown = new ArrayList<>();
    private final CustomCommand customCommand;
    private final Config config;

    public RolePlayCommand(CustomCommand customCommand, Config config) {
        super(customCommand.getName());
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String label, String[] args) {
        ...
        if (customCommand.getPermission() != null
                && !sender.hasPermission(customCommand.getPermission())) {
            sender.sendMessage(Other.color(customCommand.getNoPermission()));
            return true;
        }
